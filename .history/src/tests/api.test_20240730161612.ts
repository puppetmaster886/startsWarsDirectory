...
describe('API calls', () => {
  ...
  test('fetchPeople works correctly', async () => {
    const data = {
      results: [{ name: 'Luke Skywalker', homeworld: 'https://swapi.dev/api/planets/1/', species: ['https://swapi.dev/api/species/1/'] }],
      next: null,
    };
    ...
    expect(mockedAxios.get).toHaveBeenCalledWith('https://swapi.dev/api/people/', { params: { page: 1, search: '' } });
  });
